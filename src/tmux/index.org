...
*** Status line
The bottom-of-screen status bar. Left-align the windows.
#+begin_src conf
...
Colors follow the terminal theme: =bg= / =fg= use the terminal default; the
...
Mode indicator styles — each mode maps to an ANSI accent from the
terminal theme: prefix → =blue=, copy → =yellow=, sync → =red=,
empty → =green=. Background is =default= so the chip blends with
the bar; foreground is the accent color so the text carries the
theme color while staying legible.
#+begin_src conf
